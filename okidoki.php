...
# http://php.net/manual/en/function.file-get-contents.php
	...
	//soov
	/*foreach($html->find('li[class=classified]') as $element1){

			$href1 = "http://www.okidoki.ee".$element1->find('h3 a')[0]->href;
			//echo $href.'<br>';

			$title1 =$element1->find('h3 a')[0]->innertext. '<br>';
			//echo $title;

			$page_contents1 = str_get_html(file_get_contents($href1));
			$desc1 = $page_contents1->find('div[id=description-content]')[0];
			//echo $desc;

			$image1 = $page_contents1->find('a img')[0];
			//echo $image.'<br>';
...
$searches = array();
	...
	foreach($html->find('li[class=classified]') as $element){
			...
			$desc = $page_contents->find('div[id=description-content]')[0]->innertext;
			//echo $desc;
