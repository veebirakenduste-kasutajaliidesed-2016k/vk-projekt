/*
	TODO: Done - Remaining particles from rows % columns
	TODO: Caching/offline
	TODO: Async load images, stream caching? Download .mp3 into localStorage?
	TODO: Semi-done - Scene generation revamp - attach arrays to scene?
	TODO: Done (attached to scene.tracks) - In createParticles() attach programs to something other than a global array.
	TODO: WebGL renderer support
	TODO: Attempt to support old browsers (canvas support and HTML5 is minimum, no Flash)
	TODO: Playlist selector interface, currently playing Array[0]
...
function generate(permalink) {
    SC.get('/playlists', {
        ...
        if (e.length > 0) {
            clearView();
            SC.get('/playlists/' + e[0].id, {}).then(function(playlist) {
                createTracks(playlist);
            });
        } else {
            console.log("NOPE");
